...
def setup_underactuated():
    ...
    try:
        ...
        if platform.system() == "Darwin":
            get_ipython().system(
                u"if [ ! -d '/opt/underactuated' ]; then git clone https://github.com/j-samayoa/underactuated.git /opt/underactuated && /opt/underactuated/scripts/setup/mac/install_prereqs; fi"  # noqa
            )
        elif platform.linux_distribution() == ("Ubuntu", "18.04", "bionic"):
            get_ipython().system(
                u"if [ ! -d '/opt/underactuated' ]; then git clone https://github.com/j-samayoa/underactuated.git /opt/underactuated && /opt/underactuated/scripts/setup/ubuntu/18.04/install_prereqs; fi"  # noqa
            )
        else:
            assert False, "Unsupported platform"
